@testset "Geo interface" begin
    ...
    @test GeoInterphase.geomtype(pt) == GeoInterface.Point()
    ...
    @test GeoInterphase.geomtype(mpt) == GeoInterface.MultiPoint()

    coords = Vector{Float64}[[8,1],[9,1],[9,2],[8,2]]
    ls = LibGEOS.LineString(coords)
    # @test GeoInterphase.coordinates(ls) == coords
    @test GeoInterphase.geomtype(ls) == GeoInterface.LineString()
    ...
    @test GeoInterphase.geomtype(mls) == GeoInterface.MultiLineString()

    coords = Vector{Float64}[[8,1],[9,1],[9,2],[8,2],[8,1]]
    lr = LibGEOS.LinearRing(coords)
    # @test GeoInterphase.coordinates(lr) == coords
    @test GeoInterphase.geomtype(lr) == GeoInterface.LineString()
    ...
    @test GeoInterphase.geomtype(polygon) == GeoInterface.Polygon()
    ...
    @test GeoInterphase.geomtype(multipolygon) == GeoInterface.MultiPolygon()
    ...
    @test GeoInterphase.geomtype(geomcollection) == GeoInterface.GeometryCollection()
